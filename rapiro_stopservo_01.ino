/*
Original code by Shota Ishiwatari can be found at www.kiluck.co.jp/rapiro/RAPIRO_ver0_0.ino or www.rapiro.com/downloads/.

NEW: A "HALT" command is added to stop the servos. Just send #H over serial connection.
The code is prepared to add more motions/moves. MAXMN has to be adapted according to the actual number of motions/moves in this sketch.
Instead of sending #M0, #M1, ..., it now must read #M00, #M01, .., #M10 and so on.
Bug fixed for code lines for "case 'S'".
Be careful, this code might still have bugs! Take it as example for own programming.
*/
